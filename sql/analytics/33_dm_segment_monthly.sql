WITH contextual AS (
    SELECT
        c.*,
        CASE WHEN c.event_month = u.first_event_month THEN 'new' ELSE 'returning' END AS lifecycle_segment
    FROM analytics.fct_search_contexts c
    LEFT JOIN analytics.dim_user_first_seen u USING (user_id)
), segmented AS (
    SELECT event_month, user_id, has_booking, interaction_rows, weighted_interactions,
...
ORDER BY event_month, segment_type, segment_value
